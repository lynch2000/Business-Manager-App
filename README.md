...
2. In **Project Settings → API**, copy the **Project URL** and **anon public
   key**.
3. Open the **SQL Editor** and run, in order, everything in
   [`supabase/migrations/`](supabase/migrations) — currently `0001_init.sql`,
   `0002_expenses.sql`, and `0003_creditor_receipts.sql`. Together these
   create all tables, row-level security policies, the document numbering
   function, and the `logos` and `receipts` storage buckets.
...
## 4. Set up receipt scanning (Expenses & Accounts)
...
app. It uploads to Supabase Storage and, if configured, sends it to Claude
...
**If you skip this step**, photo/file capture and storage still work — the
app just leaves the vendor/amount/date fields blank for you to fill in by
hand instead of pre-filling them.
